...
class ClassicFloorModV4C:
    name = "classic_floor_mod_v4c"
    warmup_candles = 22
    ...
    def generate_signals(
        ...
        """
        ClassicFloorModV4A: Strategy entering on 3rd candle's open from signal,
        with dynamic delay & confirmation filters, 6 candlestick patterns,
        and Setup Cancellation on entry_1 = 'DR-UG-UG' + epcpatt_1 = 'Bull_Spinning_Top':
        ...
        - Cancellation Rule:
          If pre-entry 3 candles (entry_1) is 'DR-UG-UG' and previous candle pattern (epcpatt_1) is 'Bull_Spinning_Top',
          CANCEL the entry immediately without entering a position.
          Mark exit_reason = 'canceld' and cancel_rule = 'Bull_Spinning_Top_on_DR-UG-UG'.
        ...
        cancel_rule_series = np.full(n, None, dtype=object)
        ...
        for i in range(n):
            ...
            if waiting_for_entry and i == current_target_bar:
                ...
                # 8. Cancel Rule: do not enter on entry_1 = 'DR-UG-UG' and epcpatt_1 = 'Bull_Spinning_Top'
                if df['entry_1'].iloc[i] == 'DR-UG-UG' and patt_df['epcpatt_1'].iloc[i] == 'Bull_Spinning_Top':
                    waiting_for_entry = False
                    checking_drugdr = False
                    checking_drdrug = False
                    checking_ugdrug = False
                    ...
                    target_price = None
                    entry_price_val = None
                    exit_reason_series[i] = "canceld"
                    cancel_rule_series[i] = "Bull_Spinning_Top_on_DR-UG-UG"
                    signal_time_series[i] = signal_time_val
                    continue
                ...
                # Check touches on entry bar itself (after open)
                for j in range(5):
                    eps = max(fib_levels[j] * 1e-7, 1e-9)
        ...
        trades_df['cancel_rule'] = cancel_rule_series
