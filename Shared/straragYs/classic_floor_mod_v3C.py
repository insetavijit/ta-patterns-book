...
class ClassicFloorModV3C:
    name = "classic_floor_mod_v3c"
    warmup_candles = 22
    ...
    def generate_signals(
        ...
        """
        ClassicFloorModV3C: Strategy entering on 3rd candle's open from signal,
        with dynamic delay & confirmation filters:
        ...
        for i in range(n):
            ...
            if waiting_for_entry and (current_target_bar is not None) and (i == current_target_bar):
                # 1. If we previously delayed 1 candle specifically to check DR-DR-UG confirmation
                if checking_drdrug:
                    checking_drdrug = False
                    # Check if the waiting candle closed as DR-UG-DR (down-red)
                    if df['entry_1'].iloc[i] == 'DR-UG-DR':
                        current_target_bar = i + 1  # Delay 1 more candle -> enters at Signal + 5th candle Open
                        continue
                    # Else green confirmation printed -> proceed to enter below

                # 2. If we previously delayed 1 candle specifically to check DR-UG-DR confirmation
                if checking_drugdr:
                    checking_drugdr = False
                ...
                # 3. Check if pre-entry 3-candle setup (entry_1) is DR-DR-DR
                if df['entry_1'].iloc[i] == 'DR-DR-DR':
                    current_target_bar = i + 3  # Delay entry by 3 more candles
                    continue

                # 4. Check if pre-entry 3-candle setup (entry_1) is DR-UG-DR
                if df['entry_1'].iloc[i] == 'DR-UG-DR':
                    checking_drugdr = True
                    current_target_bar = i + 1  # Hold for 1 candle to form and confirm
                    continue

                # 5. Check if pre-entry 3-candle setup (entry_1) is DR-DR-UG
