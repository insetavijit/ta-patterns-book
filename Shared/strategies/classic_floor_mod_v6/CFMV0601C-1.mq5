//+------------------------------------------------------------------+
//|                                                 CFMV0601C-1.mq5  |
//|                 Ported from Python: CFMV0601C-1.py               |
//|                                  Copyright 2026, Antigravity AI  |
//+------------------------------------------------------------------+
#property copyright   "Copyright 2026, Antigravity AI"
#property link        ""
#property version     "6.10"
#property description "CFMV0601C-1 - Classic Floor Trader Mod V6.1 Concurrent Non-Blocking Strategy with Dynamic SL Adaptation"
...
input int    InpMaxConcurrentPositions  = 50;            // Max allowed simultaneous open positions
...
input ulong  InpMagicNumber             = 610003;        // Magic Number
input ulong  InpSlippage                = 10;            // Allowed Slippage (points)
input string InpTradeComment            = "CFMV0601C-1"; // Order Comment

input group "=== Telemetry Export ==="
input string InpExportResultsJSON       = "CFMV0601C-1_results.json"; // Output JSON filename in Common/Files
input string InpExportOHLCVCSV          = "CFMV0601C-1_ohlcv.csv";   // Output OHLCV CSV filename in Common/Files

//--- Pending Setup Structure (Supports Concurrent Queue)
struct SPendingSetup
  {
   datetime signal_time;          // Timestamp of signal candle
   int      bars_elapsed;         // Bars elapsed since signal
   double   signal_close;         // Close of signal bar
...
// Dynamic array for concurrent pending setups
SPendingSetup  g_pending_setups[];
...
void   AddPendingSetup(const SPendingSetup &setup);
void   RemovePendingSetup(const int index);
...
int OnInit()
  {
   ...
   ArrayResize(g_pending_setups, 0);
   g_test_start_bar = 0;
   g_last_bar_time  = 0;

   PrintFormat("CFMV0601C-1 (Concurrent) initialized on %s %s. Lookback=%d, ConfirmationBars=%d, Magic=%d",
...
void OnDeinit(const int reason)
  {
   ...
   PrintFormat("CFMV0601C-1 deinitialized. Reason code: %d", reason);
  }
...
//+------------------------------------------------------------------+
//| Add a new setup to the concurrent queue                          |
//+------------------------------------------------------------------+
void AddPendingSetup(const SPendingSetup &setup)
  {
   int size = ArraySize(g_pending_setups);
   ArrayResize(g_pending_setups, size + 1);
   g_pending_setups[size] = setup;
  }

//+------------------------------------------------------------------+
//| Remove a setup from the concurrent queue                         |
//+------------------------------------------------------------------+
void RemovePendingSetup(const int index)
  {
   int size = ArraySize(g_pending_setups);
   ...
   for(int i = index; i < size - 1; i++)
     {
      g_pending_setups[i] = g_pending_setups[i + 1];
     }
   ArrayResize(g_pending_setups, size - 1);
  }

//+------------------------------------------------------------------+
//| Count active open positions managed by this EA                   |
//+------------------------------------------------------------------+
int GetOpenPositionsCount()
  {
   int count = 0;
...
//+------------------------------------------------------------------+
//| Expert tick function (Bar-by-Bar Execution)                      |
//+------------------------------------------------------------------+
void OnTick()
  {
   datetime current_bar_time = (datetime)SeriesInfoInteger(_Symbol, _Period, SERIES_LASTBAR_DATE);
   if(current_bar_time == g_last_bar_time)
      return; // New bar check
   ...
   // ------------------------------------------------------------------
   // Step 1: Manage Concurrent Pending Setups (Bar +3 Open Execution)
   // ------------------------------------------------------------------
   int num_setups = ArraySize(g_pending_setups);
   for(int s = num_setups - 1; s >= 0; s--)
     {
      g_pending_setups[s].bars_elapsed++;

      // Update swing body low across elapsed confirmation bars
      double closed_body_low = MathMin(rates[1].open, rates[1].close);
      if(closed_body_low < g_pending_setups[s].swing_body_low)
         g_pending_setups[s].swing_body_low = closed_body_low;

      if(g_pending_setups[s].bars_elapsed >= InpConfirmationBars)
        {
         // Concurrent capacity gate
         if(GetOpenPositionsCount() < InpMaxConcurrentPositions)
           {
            double entry_price = rates[0].open;
            double target_tp   = g_pending_setups[s].upper_pivot;
            double pivot_sl    = g_pending_setups[s].lower_pivot;

            double half_range  = 0.5 * (g_pending_setups[s].upper_pivot - g_pending_setups[s].lower_pivot);
            double safe_sl     = g_pending_setups[s].swing_body_low - half_range;
            ...
            // Core Dynamic SL Adaptation Rule
            if(InpEnableDynamicSLAdapt && proj_rr_safe < InpSubRRThreshold && pivot_sl < entry_price)
              {
               active_sl = pivot_sl;
               sl_mode   = "PIVOT";
              }
            ...
            PrintFormat("CONCURRENT ENTRY TRIGGER: Bar+%d reached. Mode=%s, Entry=%.5f, SL=%.5f, TP=%.5f, Proj_RR=%.2f, Lots=%.2f",
                        InpConfirmationBars, sl_mode, entry_price, active_sl, target_tp, proj_rr_safe, lots);

            if(g_trade.Buy(lots, _Symbol, entry_price, active_sl, target_tp, comment))
              {
               PrintFormat("ORDER PLACED: Ticket #%I64u | Mode: %s | Lots: %.2f | Active Pos: %d",
                           g_trade.ResultOrder(), sl_mode, lots, GetOpenPositionsCount());
              }
            else
              {
           ...
           {
            PrintFormat("SETUP SKIPPED: Max concurrent positions capacity (%d) reached.", InpMaxConcurrentPositions);
           }

         // Remove executed/expired setup from queue
         RemovePendingSetup(s);
        }
     }

   // ------------------------------------------------------------------
   // Step 2: Detect New Setup Signal at Bar 1 (Concurrent Non-Blocking)
   // ------------------------------------------------------------------
   double high20     = 0.0;
   ...
   double lower_pivot = (pivot * 2.0) - high20; // S1
   double upper_pivot = (pivot * 2.0) - low20;  // R1

   double bar1_close  = rates[1].close;
   bool   is_active   = InpFilterZeroVolume ? (rates[1].tick_volume > 0 && rates[1].high > rates[1].low) : true;

   // Concurrent setup trigger: add to queue without blocking on existing setups/positions
   if(bar1_close <= lower_pivot && is_active)
     {
      SPendingSetup new_setup;
      new_setup.signal_time    = rates[1].time;
      new_setup.bars_elapsed   = 0;
      new_setup.signal_close   = bar1_close;
      new_setup.pivot          = pivot;
      new_setup.lower_pivot    = lower_pivot;
      new_setup.upper_pivot    = upper_pivot;
      new_setup.swing_body_low = MathMin(rates[1].open, rates[1].close);

      AddPendingSetup(new_setup);

      PrintFormat("CONCURRENT SIGNAL [Bar 1]: Close=%.5f <= LowerPivot=%.5f | Latched R1=%.5f. Pending queue size: %d",
                  bar1_close, lower_pivot, upper_pivot, ArraySize(g_pending_setups));
     }
...
void ExportBacktestResultsJSON()
  {
   ...
   json += StringFormat("    \"max_concurrent_positions\": %d\n", InpMaxConcurrentPositions);
   json += "  },\n";
         ...
         if(ticket > 0)
           {
            ...
            json += StringFormat("      \"position_id\": %I64u,\n", pos_id);
