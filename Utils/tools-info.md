...
| **MT5 Data Downloader & Resampler** | [`Utils/mt5-dn.py`](file:///home/avijit/workSpace/Code/ta-patterns-book/Utils/mt5-dn.py) | `uv run python Utils/mt5-dn.py --help` | Downloads historical date ranges from MT5, resamples via Polars, and exports to `{symbol}-{tf}-{range}.parquet`. | `uv run python Utils/mt5-dn.py --instrument EURUSD --start 2026-06-01 --end 2026-09-01 --tf 5m` |
| **Python Files Tree Tool** | [`Utils/tree.py`](file:///home/avijit/workSpace/Code/ta-patterns-book/Utils/tree.py) | `uv run python Utils/tree.py --help` | Prints stdio list of relative paths for all .py files in project with tree/table formatting options. | `uv run python Utils/tree.py` |
