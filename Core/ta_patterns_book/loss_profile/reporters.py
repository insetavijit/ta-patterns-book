...
def generate_monthly_table(
    ...
    query = build_monthly_query(view_name=view_name, target_month=target_month, has_month_col=has_month)
...
def generate_weekly_table(db_path: str, view_name: str = "trades", output_fmt: str = "text"):
    con = get_db_connection(db_path, read_only=True)
    query = build_weekly_query(view_name=view_name)
...
def generate_loss_group_table(
    ...
    con = get_db_connection(db_path, read_only=True)
    query = build_loss_group_query(view_name=view_name, pattern_filter=pattern_filter)
