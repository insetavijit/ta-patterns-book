...
def persist_trade_table(
    ...
    strategy_name: str = "classic_floor_mod_v5",
) -> None:
    """Persist all trade records into target DuckDB table with strategy-specific schema."""
    if not all_trades:
        return
    ...
    is_v6 = "v6" in strategy_name.lower() or any("pfib15_bsl" in t for t in all_trades[:5])
    ...
    for idx, t in enumerate(all_trades, start=1):
        ...
        if is_v6:
            # PURE CANONICAL v6 SCHEMA: 0 legacy aliases, includes 9 pfib metrics + concurrent_trades_count
            row = {
                "uid": idx,
                "trade_id": idx,
                "vbt_trade_id": idx,
...
def main() -> None:
    ...
    console = Console()
    target_db = args.output_db
    if not target_db or (target_db == "Shared/Data/classic_floor_mod-v5-1.duckdb" and "v6" in args.strategy.lower()):
        target_db = "Shared/Data/classic_floor_mod-v6-1.duckdb"
    ...
    persist_trade_table(target_db, "trades_concurrent_false", trades_false, strategy_name=args.strategy)
    ...
    persist_trade_table(target_db, "trades_concurrent_true", trades_true, strategy_name=args.strategy)
